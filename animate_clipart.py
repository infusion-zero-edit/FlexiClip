...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        frames_tensor_st, frames_svg_st, frames_tensor, frames_svg, points_init_frame, _, _, _, _ = painter.render_frames_to_tensor_with_bezier(mlp=cfg.opt_bezier_points_with_mlp)
    ...
    for step in t_range:
        for curr_sds_loss, opt_kwargs in sds_losses_and_opt_kwargs:
            ...
            vid_tensor, frames_svg, vid_tensor_p, frames_svg_p, new_points, shifted_locations, point_bezier, final_jacobs, pose_jacobs  = painter.render_frames_to_tensor_with_bezier(mlp=cfg.opt_bezier_points_with_mlp)  # (K, 256, 256, 3)
            x = vid_tensor.unsqueeze(0).permute(0, 1, 4, 2, 3)  # (K, 256, 256, 3) -> (1, K, 3, 256, 256)
            x_p = vid_tensor_p.unsqueeze(0).permute(0, 1, 4, 2, 3)  # (K, 256, 256, 3) -> (1, K, 3, 256, 256)

            x = x.repeat(cfg.batch_size, 1, 1, 1, 1)
            x_p = x_p.repeat(cfg.batch_size, 1, 1, 1, 1)
            ...
            if cfg.augment_frames:
                ...
                augmented_pair_p = data_augs(torch.cat([x_p.squeeze(0), orig_frames.squeeze(0)]))
                x_aug_p = augmented_pair_p[:cfg.num_frames].unsqueeze(0)
            else:
                x_aug = x
                orig_frames_aug = orig_frames
            
            loss_sds_p, logp_p = curr_sds_loss(x_aug_p, **loss_kwargs)
            loss_sds_p = loss_sds_p.detach()
            logp_p = logp_p.detach()
            
            loss_sds, logp = curr_sds_loss(x_aug, **loss_kwargs)
            
            loss = loss_sds
            loss += torch.abs(nnf.l1_loss(final_jacobs, pose_jacobs.detach(), reduction="mean")) + nnf.mse_loss(logp, logp_p, reduction='mean')  
        ...
        if step % cfg.save_vid_iter == 0:
            util.save_mp4_from_tensor(vid_tensor_p, f"{cfg.output_folder}/mp4_logs/{step}.mp4")
