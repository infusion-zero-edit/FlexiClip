...
class SDSLossBase(nn.Module):
    ...
    def __init__(self, cfg, device, reuse_pipe=True):
        ...
        self.final_alpha_cumprod = self.alphas[-1]  # Alpha for t=0, end of cumulative product
    ...
    def sds_grads(self, latent_z, **sds_kwargs):
        ...
        with torch.no_grad():
            ...
            prev_timestep = torch.clamp(timestep - 1, 0, self.cfg.timesteps - 1).to(self.device)
        
            # Gather alphas
            alpha_prod_t = self.alphas.gather(0, timestep).to(self.device)
            alpha_prod_t_prev = torch.where(
                prev_timestep >= 0,
                self.alphas.gather(0, prev_timestep),
            ...
            pred_original_sample = (noised_latent_zt - beta_prod_t ** 0.5 * eps_t) / alpha_prod_t ** 0.5
            variance = ((1 - alpha_prod_t_prev) / (1 - alpha_prod_t)).sqrt() * ((1 - alpha_prod_t / alpha_prod_t_prev).sqrt())
            std_dev_t = 1.0 * variance

            # Calculate previous sample mean
            pred_sample_direction = (1 - alpha_prod_t_prev - std_dev_t**2).sqrt() * eps_t
            prev_sample_mean = alpha_prod_t_prev.sqrt() * pred_original_sample + pred_sample_direction

            # Noise addition to get `prev_sample`
            variance_noise = torch.randn_like(prev_sample_mean, device=self.device, dtype=prev_sample_mean.dtype)
            prev_sample = prev_sample_mean + std_dev_t * variance_noise

            # Log probability calculation
            log_prob = -((prev_sample - prev_sample_mean) ** 2) / (2 * (std_dev_t**2)) \
                       - torch.log(std_dev_t) \
                       - torch.log(torch.sqrt(torch.tensor(2 * math.pi, device=std_dev_t.device, dtype=std_dev_t.dtype)))
        
            #Average or sum log prob over dimensions, based on `logp_mean`
            # log_prob = log_prob.mean(dim=tuple(range(1, log_prob.ndim)))[0]

            return grad_z, log_prob
...
class SDSVideoLoss(SDSLossBase):
    ...
    def forward(self, x_aug, grad_scale=1.0):
        ...
        grad_z, logp = self.sds_grads(latent_z)
        grad_z = grad_scale * grad_z
